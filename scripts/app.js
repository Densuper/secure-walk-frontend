 ...
 * @param {string} [expectedRole=null] - Optional. If provided, login will also check if user's role matches.
 * @returns {object} An object with success status, and role or message.
 *                   e.g., { success: true, role: 'admin' } or { success: false, message: 'Error details' }
 */
function loginUser(username, password, expectedRole = null) {
  console.log(`Attempting login for user: ${username}, expecting role: ${expectedRole || 'any'}`);
  ...
  if (typeof getUsers !== 'function') {
    ...
    // Clear any potentially lingering currentUser from a previous session if system error occurs
  ...
  if (!user || user.password !== password) { // Basic password check
    // Clear any potentially lingering currentUser
    localStorage.removeItem('currentUser'); 
    const failMsg = 'Invalid username or password.';
    console.log(`Login failed for user: ${username}. ${failMsg}`);
    // Log event for failed login attempt (username might be incorrect, so log attempt against entered username)
    // logEvent(username, 'LOGIN_FAIL', failMsg); // Let calling page handle logging for more context
    return { success: false, message: failMsg };
  }

  // User authenticated, now check role if expectedRole is provided
  if (expectedRole && user.role !== expectedRole) {
    // Clear any potentially lingering currentUser
    localStorage.removeItem('currentUser'); 
    const roleMismatchMsg = `User '${username}' authenticated but has role '${user.role}', expected '${expectedRole}'. Access denied.`;
    console.warn(roleMismatchMsg);
    // Log event for role mismatch after successful authentication
    // logEvent(username, 'LOGIN_ROLE_MISMATCH', roleMismatchMsg); // Let calling page handle logging
    return { success: false, message: roleMismatchMsg };
  }

  // All checks passed (auth + optional role match)
  ...
  // Let the calling page (admin-login.html or user-login.html) handle specific success logging
